...
function multiavatar (string, sansEnv, ver, gender) {
  string += '';

  let pre = string.substring(0, 3);
  if (pre == '[f]') {
    gender = 'female';
  ...
  */
  ...
  //   if (gender == 'male') {
  //     // check p['mouth'] and p['top'] are available in female['mouth'] and female['top'] arrays, then pick the next part
  //     // e.g. if part is p['mouth'] = '03A' is set, and since it's available in female['mouth'], then increase the part number by 1, so that would be p['mouth'] = '04A' 
  //   }
  //   else if (gender == 'famale') {
  //     // do similar like for male
  //     // in case the next part is also male, like in 11A and 12A, then the part needs to be set to 13A
  //     // in case the number is 15, as in 15A, 15B, 15C, then ne next part needs to be not 16 but start from beginning at 0
  //   }
  // }
  male = [];
  male['mouth'] = [ '04A', '04B', '04C', '07A', '07B', '07C', '10A', '10B', '11A', '11B', '11C', '12A', '12B', '12C', '13A', '13B', '13C', '14A', '14B', '14C' ];
  male['top'] = [ '04A', '04B', '04C', '05C', '10A' ];
  
  female = [];
  female['mouth'] = [ '02A', '02B', '02C' ];
  female['top'] = [ '02A', '02C' ];
  
  if (typeof(gender) !== 'undefined') {
    if (gender === 'male') {
      // Check and update mouth
      if (female['mouth'].includes(p['mouth'])) {
        let partNum = parseInt(p['mouth'].substring(0, 2), 10); // Extract number
        let suffix = p['mouth'][2]; // Extract letter
        do {
          partNum += 1; // Increment number
          if (partNum > 15) partNum = 0; // Reset to 0 if beyond range
          p['mouth'] = (partNum < 10 ? '0' : '') + partNum + suffix; // Generate new part
        } while (female['mouth'].includes(p['mouth'])); // Check if valid
      }
  
      // Check and update top
      if (female['top'].includes(p['top'])) {
        let partNum = parseInt(p['top'].substring(0, 2), 10); // Extract number
        let suffix = p['top'][2]; // Extract letter
        do {
          partNum += 1; // Increment number
          if (partNum > 15) partNum = 0; // Reset to 0 if beyond range
          p['top'] = (partNum < 10 ? '0' : '') + partNum + suffix; // Generate new part
        } while (female['top'].includes(p['top'])); // Check if valid
      }
    } else if (gender === 'female') {
      // Check and update mouth
      if (male['mouth'].includes(p['mouth'])) {
        let suffix = p['mouth'][2]; // Extract the letter (A, B, C)
        let newMouth;
        if (suffix === 'A') newMouth = '02A';
        else if (suffix === 'B') newMouth = '02B';
        else if (suffix === 'C') newMouth = '02C';
        p['mouth'] = newMouth; // Assign the new mouth part
      }
  
      // Check and update top
      if (male['top'].includes(p['top'])) {
        let partNum = parseInt(p['top'].substring(0, 2), 10); // Extract number
        let suffix = p['top'][2]; // Extract letter
        do {
          partNum += 1; // Increment number
          if (partNum > 15) partNum = 0; // Reset to 0 if beyond range
          p['top'] = (partNum < 10 ? '0' : '') + partNum + suffix; // Generate new part
        } while (male['top'].includes(p['top'])); // Check if valid
      }
    }
  }
